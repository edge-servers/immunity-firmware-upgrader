...
from immunity_controller.connection.connectors.exceptions import CommandFailedException
from immunity_controller.connection.connectors.openwrt.ssh import (
    OpenWrt as OpenWrtSshConnector,
)
from immunity_controller.connection.exceptions import NoWorkingDeviceConnectionError
from immunity_controller.connection.tests.utils import SshServer
...
def mocked_exec_upgrade_success(command, exit_codes=None, timeout=None):
    ...
    cases = {
        ...
        'mkdir -p /etc/immunity': defaults,
        f'echo {TEST_CHECKSUM} > {_checksum}': defaults,
        f'{_sysupgrade} --help': ['--test', 1],
        'rm /etc/immunity/checksum 2> /dev/null': defaults,
...
class TestOpenwrtUpgrader(TestUpgraderMixin, TransactionTestCase):
    ...
    @patch('immunity_controller.connection.settings.MANAGEMENT_IP_ONLY', False)
    @patch.object(OpenWrt, '_call_reflash_command')
    @patch('scp.SCPClient.putfo')
    @patch.object(OpenWrt, 'RECONNECT_DELAY', 0)
    @patch.object(OpenWrt, 'RECONNECT_RETRY_DELAY', 0)
        ...
        with self.subTest('success'):
            with patch.object(
                ...
                self.assertEqual(
                    ...
                    ('rm /etc/immunity/checksum 2> /dev/null',),
                )
                self.assertEqual(
