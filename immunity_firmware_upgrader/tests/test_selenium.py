...
from immunity_firmware_upgrader.hardware import REVERSE_FIRMWARE_IMAGE_MAP
from immunity_firmware_upgrader.tests.base import TestUpgraderMixin
...
from ..upgraders.immunity import OpenWrt
...
class TestDeviceAdmin(TestUpgraderMixin, SeleniumTestMixin, StaticLiveServerTestCase):
    ...
    @capture_any_output()
    @patch(
        'immunity_firmware_upgrader.upgraders.openwrt.OpenWrt.upgrade',
        return_value=True,
    )
    @patch(
        'immunity_controller.connection.models.DeviceConnection.connect',
        return_value=True,
    )
